...
const { prependOnceListener } = require("./models/BookModel");
...
// Book.find(({rating : {$lt : 5 }}).sort({ author : -1}).exec(function(err , Book){
// }))
...
People.find({
  kids: { $elemMatch: { hair: "gray" } }}).exec(function (err, People) {});

//5. Find all the people who have at least one overweight kid, and are overweight themselves (>100)

People.find({
  kids: { $elemMatch: { weight: {$gt : 100} } }}).exec(function (err, People) {});